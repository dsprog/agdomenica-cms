                    ...
                    <div class="p-8">
                        <form action="{{ route('admin.banners.update', $banner->id) }}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="w-full mb-5">
                                ...
                                <x-jet-input name="title" type="text" value="{{ $banner->title }}" />
                                <x-jet-input-error for="title" />
                            </div>
                            <div class="w-full mb-5">
                                <label class="block">Link</label>
                                <x-jet-input name="url" type="text" value="{{ $banner->url }}" />
                            ...
                            <div class="w-full md:w-2/5 mb-5">
                                ...
                                <x-jet-input name="published_at" type="date" value="{{ $banner->published_at }}" />
                                <x-jet-input-error for="published_at" />
                            </div>
                            <div class="w-full md:w-2/5 mb-5">
                                <label class="block">Data de expiração (não obrigatório)</label>
                                <x-jet-input name="expiration_date" type="date" value="{{ $banner->expiration_date }}" />
                                <x-jet-input-error for="expiration_date" />
                            </div>
                            <div class="mb-5">
                                <label class="block">Ativo</label>
                                <input type="radio" name="active" value="1" {{ ($banner->active == 1) ? 'checked':'' }}> Sim &nbsp;&nbsp;
                                <input type="radio" name="active" value="0" {{ ($banner->active == 0) ? 'checked':'' }}> Não
                            </div>
