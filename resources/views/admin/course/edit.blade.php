                    ...
                    <div class="p-8">
                        <form action="{{ route('admin.courses.update', $course->id) }}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="w-full mb-5">
                                ...
                                <x-jet-input name="title" type="text" value="{{ $course->title }}" />
                                <x-jet-input-error for="title" />
                            </div>
                            <div class="w-full mb-5">
                                <label class="block">Data do curso - ex (00/00/0000 ou em breve etc)</label>
                                <x-jet-input name="date_info" type="text" value="{{ $course->date_info }}" />
                                <x-jet-input-error for="date_info" />
                            </div>
                            <div class="w-full mb-5">
                                <label class="block">Resumo</label>
                                <x-jet-input name="resume" type="text" value="{{ $course->resume }}" />
                                <x-jet-input-error for="resume" />
                            </div>
                            <div class="w-full mb-5">
                                <label class="block">Inscrição</label>
                                <x-jet-input name="subscription" type="text" value="{{ $course->subscription }}" />
                                <x-jet-input-error for="subscription" />
                            </div>
                            <div class="w-full">
                                <label class="block">Período</label>
                                <textarea name="period" id="period" cols="30" rows="10">{{ $course->period }}</textarea>
                                <x-jet-input-error for="period" />
                            </div>
                            <div class="mb-5"></div>
                            <div class="w-full">
                                <label class="block">Investimento</label>
                                <textarea name="investment" id="investment" cols="30" rows="10">{{ $course->investment }}</textarea>
                                <x-jet-input-error for="investment" />
                            </div>
                            <div class="mb-5"></div>
                            <div class="w-full">
                                <label class="block">Mais informações</label>
                                <textarea name="more_info" id="more_info" cols="30" rows="10">{{ $course->more_info }}</textarea>
                                <x-jet-input-error for="more_info" />
                            </div>
                            <div class="mb-5"></div>
                            <div class="w-full">
                                <label class="block">Descrição</label>
                                <textarea name="body" id="body" cols="30" rows="10">{{ $course->body }}</textarea>
                            ...
                            <div class="w-full md:w-2/5 mb-5">
                                ...
                                <x-jet-input name="published_at" type="date" value="{{ $course->published_at }}" />
                                <x-jet-input-error for="published_at" />
                            </div>
                            <div class="mb-5">
                                <label class="block">Ativo</label>
                                <input type="radio" name="active" value="1" {{ ($course->active == 1) ? 'checked':'' }}> Sim &nbsp;&nbsp;
                                <input type="radio" name="active" value="0" {{ ($course->active == 0) ? 'checked':'' }}> Não
                            </div>
